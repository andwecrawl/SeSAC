    ...
    @IBAction func searchTextField(_ sender: UITextField) {
        ...
        if let word = sender.text?.uppercased() {
            if let result = coinedWords[word] {
                ...
                let alert = UIAlertController(title: "올바른 단어를 입력해 주세요!", message: "문자열이 비어 있거나 스펠링이 잘못되었습니다.", preferredStyle: .alert)
...
//                meansTextLabel.text = "올바른 단어를 입력해 주세요!"
            ...
            let alert = UIAlertController(title: "올바른 단어를 입력해 주세요!", message: "문자열이 비어 있습니다.", preferredStyle: .alert)
            let okay = UIAlertAction(title: "okay", style: .default)
            alert.addAction(okay)
            present(alert, animated: true)
//            meansTextLabel.text = "단어를 입력해 주세요!"
